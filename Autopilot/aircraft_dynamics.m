...
function sys = mdlDerivatives(t, x, uu, P)
    ...
    neddot = [...
        cos(theta)*cos(psi) sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi) c_phi*s_tht*c_psi+s_phi*s_psi;...
        c_tht*s_psi s_phi*s_tht*s_psi+c_phi*c_psi c_phi*s_tht*s_psi-s_phi*c_psi;...
        -s_tht s_phi*c_tht c_phi*c_tht]*[u; v; w] % To be completed by students
    ...
    attdot = [1 s_phi*t_tht c_phi*t_tht; 0 c_phi -s_phi; 0 s_phi/c_tht c_phi/c_tht]*[p; q; r] % To be completed by students
    ...
    attratdot = [gam1*p*q-gam2*q*r; gam5*p*r-gam6*(p^2-r^2); gam7*p*q-gam1*q*r]...
        +[gam3*ell+gam4*n; (1/P.Jy)*m; gam4*ell+gam8*n] % To be completed by students
